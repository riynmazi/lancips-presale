...
let wallet = null; const PRICE_PER_TOKEN = 0.000005; const OWNER_WALLET = "7VJHv1UNSCoxdNmboxLrjMj1FgyaGdSELK9Eo4iaPVC8"; // change
...
// ✅ Buy LANCIPS buyBtn.onclick = async () => { const sol = parseFloat(solAmountInput.value); if (!sol || sol <= 0) { alert("⚠️ Please enter a valid amount of SOL."); return; }

const tokens = sol / PRICE_PER_TOKEN; buyBtn.disabled = true; buyBtn.textContent = "Processing..."; statusMsg.textContent = "⏳ Sending transaction...";

try { const provider = window.solana; const connection = new solanaWeb3.Connection( "https://rpc.helius.xyz/?api-key=6a1332cb-869d-4794-8c3d-737a487ab1e2", "confirmed" );
...
} catch (e) { console.error(e); alert("❌ Transaction failed:\n" + e.message); statusMsg.textContent = "❌ Transaction failed."; } finally { buyBtn.disabled = false; buyBtn.textContent = "Buy LANCIPS"; } };

// ⏳ Static Countdown Timer (ends at 30 July 2025) const countdownEl = document.getElementById("countdown"); const endTime = new Date("2025-07-30T00:00:00");

function updateCountdown() { const now = new Date().getTime(); const distance = endTime.getTime() - now;

if (distance <= 0) { countdownEl.textContent = "⛔ Presale has ended."; if (buyBtn) buyBtn.disabled = true; return; }
...
countdownEl.textContent = ⏳ Presale ends in: ${days}d ${hours}h ${minutes}m ${seconds}s; }
